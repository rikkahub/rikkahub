...
private val AUTO_GENERATED_THEME_CONTENT_COLOR_PAIRS = listOf(
...
private const val MIN_THEME_CONTENT_COLOR_CONTRAST = 4.5f
...
fun ColorScheme.applyThemeTokenOverrides(
    source: String,
    autoGenerateContentColors: Boolean,
): ColorScheme {
    return applyThemeTokenOverrides(
        parseResult = parseThemeTokenSource(source),
        autoGenerateContentColors = autoGenerateContentColors,
    )
}
...
fun ColorScheme.applyThemeTokenOverrides(
    parseResult: ThemeTokenParseResult,
    autoGenerateContentColors: Boolean,
): ColorScheme {
    val effectiveParseResult = if (autoGenerateContentColors) {
        parseResult.withAutoGeneratedContentColors(baseScheme = this)
    } else {
        parseResult
    }
    val overrides = effectiveParseResult.overrides
...
fun ThemeTokenParseResult.withAutoGeneratedContentColors(baseScheme: ColorScheme): ThemeTokenParseResult {
    if (overrides.isEmpty()) {
        return this
    }

    val generatedOverrides = overrides.toMutableMap()
    var generatedAny = false

    AUTO_GENERATED_THEME_CONTENT_COLOR_PAIRS.forEach { (sourceKey, targetKey) ->
        if (overrides.containsKey(targetKey)) {
            return@forEach
        }

        val sourceColor = overrides[sourceKey] ?: return@forEach
        val effectiveSourceColor = sourceColor.compositeAgainst(baseScheme.themeTokenColor(sourceKey))
        generatedOverrides[targetKey] = effectiveSourceColor.preferredContentColor()
        generatedAny = true
    }
    ...
    return copy(overrides = generatedOverrides)
}

fun ThemeTokenParseResult.withAccessibleContentColors(
    baseScheme: ColorScheme,
    autoGenerateContentColors: Boolean,
    minimumContrastRatio: Float = MIN_THEME_CONTENT_COLOR_CONTRAST,
): ThemeTokenParseResult {
    if (autoGenerateContentColors) {
        return withAutoGeneratedContentColors(baseScheme = baseScheme)
    }
    if (overrides.isEmpty()) {
        return this
    }

    val accessibleOverrides = overrides.toMutableMap()
    var generatedAny = false
    ...
    AUTO_GENERATED_THEME_CONTENT_COLOR_PAIRS.forEach { (sourceKey, targetKey) ->
        ...
        val sourceColor = overrides[sourceKey] ?: return@forEach
        val effectiveSourceColor = sourceColor.compositeAgainst(baseScheme.themeTokenColor(sourceKey))
        val fallbackContentColor = baseScheme.themeTokenColor(targetKey)
        if (!fallbackContentColor.hasMinimumContrastAgainst(effectiveSourceColor, minimumContrastRatio)) {
            accessibleOverrides[targetKey] = effectiveSourceColor.preferredContentColor()
            generatedAny = true
        }
    }
    ...
    return copy(overrides = accessibleOverrides)
}
