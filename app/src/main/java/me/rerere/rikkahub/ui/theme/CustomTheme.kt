...
fun ColorScheme.applyThemeTokenOverrides(
    ...
    val effectiveParseResult = if (autoGenerateContentColors) {
        parseResult.withAutoGeneratedContentColors()
    } else {
        parseResult
    }
...
fun ThemeTokenParseResult.withAutoGeneratedContentColors(): ThemeTokenParseResult {
    if (overrides.isEmpty()) {
        return this
    }
    ...
    AUTO_GENERATED_THEME_CONTENT_COLOR_PAIRS.forEach { (sourceKey, targetKey) ->
        ...
        generatedOverrides[targetKey] = sourceColor.preferredContentColor()
        generatedAny = true
    }
...
fun ThemeTokenParseResult.withAccessibleContentColors(
    ...
    if (autoGenerateContentColors) {
        return withAutoGeneratedContentColors()
    }
    if (overrides.isEmpty()) {
        return this
    }
    ...
    AUTO_GENERATED_THEME_CONTENT_COLOR_PAIRS.forEach { (sourceKey, targetKey) ->
        ...
        if (!fallbackContentColor.hasMinimumContrastAgainst(sourceColor, minimumContrastRatio)) {
            accessibleOverrides[targetKey] = sourceColor.preferredContentColor()
            generatedAny = true
        }
    }
