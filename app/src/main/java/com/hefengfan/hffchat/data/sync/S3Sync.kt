package com.hefengfan.hffchat.data.sync
...
import com.hefengfan.hffchat.data.files.FileFolders
import com.hefengfan.hffchat.data.files.SkillPaths
import com.hefengfan.hffchat.data.datastore.Settings
import com.hefengfan.hffchat.data.datastore.SettingsStore
import com.hefengfan.hffchat.data.datastore.migration.SettingsJsonMigrator
import com.hefengfan.hffchat.data.sync.s3.S3Client
...
class S3Sync(
    ...
    suspend fun backupToS3(config: S3Config) = withContext(Dispatchers.IO) {
        ...
        val client = getS3Client(config)
        val key = "hffchat_backups/${file.name}"
    ...
    suspend fun listBackupFiles(config: S3Config): List<S3BackupItem> = withContext(Dispatchers.IO) {
        ...
        val result = client.listObjects(
            prefix = "hffchat_backups/",
            maxKeys = 1000
        ).getOrThrow()

        result.objects
            .filter { it.key.startsWith("hffchat_backups/backup_") && it.key.endsWith(".zip") }
            .map { obj ->
                S3BackupItem(
                    key = obj.key,
