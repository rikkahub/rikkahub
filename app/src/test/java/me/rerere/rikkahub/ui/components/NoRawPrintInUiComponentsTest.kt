...
/**
 * Static guard: the richtext and ui component layers must route diagnostics
 * through the central logging policy (android.util.Log), never through raw
 * print or Throwable.printStackTrace, which dump to System.out/System.err
 * with no tag, no severity, and bypass redaction. This mirrors
 * NoRawPrintlnInDataLayerTest (the "static grep" check #98 asks for), scoped
 * to the app/.../ui/components/richtext and app/.../ui/components/ui sources.
 */
class NoRawPrintInUiComponentsTest {
    ...
    private val scopedDirs = listOf(
        "src/main/java/me/rerere/rikkahub/ui/components/richtext",
        "src/main/java/me/rerere/rikkahub/ui/components/ui",
    )

    @Test
    fun `richtext and ui components have no raw print or printStackTrace`() {
        ...
        assertTrue(
            "Forbidden raw logging in ui components:\n" + violations.joinToString("\n"),
            violations.isEmpty()
        )
    }
