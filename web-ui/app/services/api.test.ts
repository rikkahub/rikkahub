...
import { appendWebAuthQuery, clearWebAuthToken, onWebAuthRequired, setWebAuthToken } from "~/services/api";
...
describe("web auth helpers", () => {
  it("appends a valid token to API URLs while preserving query and hash", () => {
    ...
    expect(appendWebAuthQuery("/api/conversations")).toBe("/api/conversations?access_token=token-123");
