...
**Goal:** Preserve the canonical 16 kHz fixture in real time when LiveKit negotiates a different capture sample rate, so a valid physical-device call can form a final user transcript.

**Architecture:** `LiveKitAutomationPcmSource` remains the synchronized owner of queued fixture bytes and gains a small streaming PCM16 mono converter driven by the output format reported by `AudioProcessorInterface`. `LiveKitInjectedPcmProcessor` forwards initialization and reset events to that source, while production microphone capture remains untouched because the processor is enabled only during fixture automation.

**Tech Stack:** Kotlin, LiveKit Android `AudioProcessorInterface`, PCM16 little-endian audio, kotlinx-coroutines-test, JUnit 4, Gradle Android unit tests, managed `mdev` physical-device testing.
...
### Task 1: Stream 16 kHz fixture PCM into the negotiated LiveKit capture format
...
- Produces: `LiveKitAutomationPcmSource.configureOutputFormat(sampleRateHz: Int, numChannels: Int)` and format-aware `replaceOrZero(buffer: ByteBuffer)` behavior.
...
- [ ] **Step 1: Write the failing 16-to-48 kHz regression test**

Add a test that initializes the real processor at 48 kHz mono and checks hand-derived output samples:
...
Add a test-only `pcm16(vararg samples: Int): ByteArray` helper that writes each literal sample low byte then high byte. The production mutation caught by this test is returning to one input sample per output frame, which accelerates the fixture and inserts silence.
...
Expected: FAIL because the current processor copies the two source samples once and zero-fills the remaining four output frames.
...
`configureOutputFormat` requires positive values and resets `resamplePhase` and `currentSample` only when the negotiated format changes. `replaceOrZero` reads each source sample as signed little-endian PCM16, emits it once per output channel, adds 16,000 to `resamplePhase` for each output frame, and consumes source samples while the phase is at least `outputSampleRateHz`. When no complete source sample is queued, it emits zero for that frame. Any trailing byte that cannot form an output frame is zero-filled.
...
Expected: PASS. Update existing raw-byte tests to use complete PCM16 frames; do not weaken their ordering, zero-fill, stale-run, or inactive-microphone assertions.

- [ ] **Step 5: Add channel and reset coverage**
...
Use literal PCM16 samples and exact output byte arrays. Run each new test first and confirm it fails for the missing channel/reset behavior before changing production code further.
...
git commit -m "fix: preserve LiveKit fixture sample rate"
```
